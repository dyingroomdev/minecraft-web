...
class ApiClient {
  ...
  async googleOAuthCallback(code: string, state: string): Promise<TokenResponse> {
    const params = new URLSearchParams({ code, state });
    return this.request<TokenResponse>(`/auth/google/callback?${params.toString()}`);
  }
  ...
  async getMe(): Promise<User> {
    return this.request('/me', undefined, UserSchema);
  }
